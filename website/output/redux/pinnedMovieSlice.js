...
export const pinnedMovieSlice = createSlice({
  ...
  reducers: {
    updatePinnedMovieDetails: function (state, action) {
      ...
      const index = JSON.parse(sessionStorage.getItem("pinnedMovies")).findIndex(movie => {
    ...
    unpinMovie: function (state, action) {
      ...
      let movieIndex = pinnedMovies.findIndex(pinnedMovie => pinnedMovie.movieID === movieID && pinnedMovie.name === name && pinnedMovie.description === description && pinnedMovie.releaseDate === releaseDate && pinnedMovie.imageURL === imageURL && pinnedMovie.genreID === genreID && pinnedMovie.active === active);
