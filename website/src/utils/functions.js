...
import { toggleMoviesArray } from "../redux/movieSlice";
...
export async function fetchAllMovies(dispatch, option, showInitialMovies) {
  try {
    ...
    console.log(
      "Fetching all the movies via the GET /movies API route: ",
      data
    );
    dispatch(
      toggleMoviesArray({
    ...
    console.log(error);
    alert(
      "An error occurred while fetching all the movies from the database!\nPlease try again later 😔"
    );
    return [];
  }
}
