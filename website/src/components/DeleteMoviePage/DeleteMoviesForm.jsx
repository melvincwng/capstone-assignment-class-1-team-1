...
export default function DeleteMoviesForm({
  ...
  async function deleteMultipleMovieDetailsInTheDB(deletedMovieIDsArray) {
    try {
      const payload = { movieIDs: deletedMovieIDsArray };
      ...
      console.log(
        "What is the server's response (deletion of multiple movies): ",
        responseData
      );
  ...
  async function handleMultipleDelete(event) {
    ...
    const deletedMovieIDsArray = deleteMultipleMovieDetails(
      event,
      movies,
      deleteMultipleMovies,
      setDeleteMultipleMoviesSuccess
    ...
    console.log(
      "What is the array of deleted movieIDs from sessionStorage - (array format): ",
      deletedMovieIDsArray
    );

    // Approach 2 (for FCP): Need to hit the DELETE /movies API endpoint to delete multiple movie details directly in the DB
    const multipleMoviesDeletedInDB =
      deletedMovieIDsArray &&
      (await deleteMultipleMovieDetailsInTheDB(deletedMovieIDsArray));

    if (multipleMoviesDeletedInDB) {
      alert(
        "Multiple movies deleted successfully in the sessionStorage and DB! 🎉"
      );
    }
  }
