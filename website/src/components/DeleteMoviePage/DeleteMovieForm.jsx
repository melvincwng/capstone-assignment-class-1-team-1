...
export default function DeleteMovieForm({
  ...
  async function deleteMovieDetailsInTheDB(deletedMovieIDString) {
    try {
      ...
      const response = await fetch(
        `${API_HOST}/movies/${deletedMovieIDString}`,
        requestOptions
      );
      const responseData = await response.json();
      console.log(
        "What is the server's response (deletion of movie): ",
        responseData
      );
  ...
  async function handleDelete(event) {
    ...
    const deletedMovieIDString = deleteOneMovieDetails(
      movies,
      deleteOneMovie,
      setDeleteOneMovieSuccess
    );
    ...
    console.log(
      "What is the movieID of the deleted movie from sessionStorage - (string format): ",
      deletedMovieIDString
    );

    // Approach 2 (for FCP): Need to hit the DELETE /movies/:movieID API endpoint to delete the movie details directly in the DB
    const movieDeletedInDB = await deleteMovieDetailsInTheDB(
      deletedMovieIDString
    );
