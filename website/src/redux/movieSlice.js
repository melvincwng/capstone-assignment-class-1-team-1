...
export const movieSlice = createSlice({
  ...
  reducers: {
        ...
        switch (action.payload.name) {
          ...
          case HIDE_PAST_MOVIES:
            state.value = JSON.parse(sessionStorage.getItem("movies")).filter(
              (movie) => movie.active === YES
            );
            break;
          case SHOW_PAST_MOVIES:
            state.value = JSON.parse(sessionStorage.getItem("movies")).filter(
              (movie) => movie.active === NO
            );
            break;
          case SHOW_ALL_MOVIES:
    ...
    filterMoviesArray: function (state, action) {
      ...
      if (action.payload.showInitialMovies) {
        ...
        state.value = state.value.filter(
          (movie) => movie.genreID === action.payload.genreID
        );
      } else {
        // Reset the state of the store for every new click on each option of the 'Genre' button
        state.value = JSON.parse(sessionStorage.getItem("movies"));
    ...
    deleteOneMovieDetails: function (state, action) {
      console.log(
        "BEFORE DELETION OF 1 MOVIE:",
        JSON.parse(sessionStorage.getItem("movies"))
      );
      let newMoviesArray = JSON.parse(sessionStorage.getItem("movies")).filter(
    ...
    deleteMultipleMoviesDetails: function (state, action) {
      ...
      console.log(
        "BEFORE DELETION OF MULTIPLE MOVIES:",
        JSON.parse(sessionStorage.getItem("movies"))
      );
      ...
      let newMoviesArray = JSON.parse(sessionStorage.getItem("movies")).filter(
        (movie) => !selectedMovieIDsToBeDeleted.includes(movie.movieID)
      );
    ...
    updateMovieDetails: function (state, action) {
      ...
      const index = JSON.parse(sessionStorage.getItem("movies")).findIndex(
        (movie) => {
